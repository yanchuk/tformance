...
from dataforseo_api import api_post, format_count, get_result


def main():
    parser = argparse.ArgumentParser(description="Related keywords from Google")
    parser.add_argument("keyword", help="Seed keyword")
    parser.add_argument("--location", "-loc", type=int, default=2840, help="Location code (default: 2840 = US)")
    parser.add_argument(
        "--depth", "-d", type=int, default=1, help="Search depth 1-3 (default: 1, max keywords: depth^3 * 10)"
    )
    parser.add_argument("--limit", "-l", type=int, default=50, help="Max results to display (default: 50)")
    ...
    if results:
        ...
        for result in results:
            ...
            for item in items:
                ...
                if keyword:
                    keywords.append(
                        {"keyword": keyword, "volume": volume if volume is not None else 0, "difficulty": difficulty}
                    )
    ...
    print("  Depth 1: ~10 keywords, Depth 2: ~100, Depth 3: ~1,000+")
