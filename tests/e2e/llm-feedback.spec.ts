...
test.describe('LLM Feedback System @llm-feedback', () => {
  ...
  // ==========================================================================
  // PHASE 4: POSTHOG SURVEY BUTTON TESTS
  // ==========================================================================

  test.describe('Feature: PostHog Survey Button', () => {
    test('floating feedback button is visible on PR list page', async ({ page }) => {
      await page.goto('/app/metrics/pull-requests/');
      await page.waitForLoadState('domcontentloaded');

      const feedbackBtn = page.locator('[data-testid="posthog-feedback-btn"]');
      await expect(feedbackBtn).toBeVisible();
    });

    test('floating feedback button is visible on dashboard', async ({ page }) => {
      await page.goto('/app/');
      await page.waitForLoadState('domcontentloaded');
    ...
    test('feedback button is visible on analytics page', async ({ page }) => {
      await page.goto('/app/metrics/overview/');
      await page.waitForLoadState('domcontentloaded');
    ...
    test('feedback button has correct positioning', async ({ page }) => {
      await page.goto('/app/metrics/pull-requests/');
      await page.waitForLoadState('domcontentloaded');
      ...
      const boundingBox = await feedbackBtn.boundingBox();
      if (!boundingBox) {
        test.skip();
        return;
      }

      // Should be in bottom-right corner (allow for some margin)
      const viewport = page.viewportSize();
      expect(boundingBox.x).toBeGreaterThan(viewport!.width - 150);
      expect(boundingBox.y).toBeGreaterThan(viewport!.height - 150);
    });
  });
});
