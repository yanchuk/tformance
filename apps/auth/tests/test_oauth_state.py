...
class TestJiraOAuthState(TestCase):
    """Tests for Jira OAuth state flow types."""

    def test_create_jira_onboarding_state_with_team_id(self):
        """Test creating Jira onboarding state with team_id."""
        team_id = 456
        state = create_oauth_state(FLOW_TYPE_JIRA_ONBOARDING, team_id=team_id)

        self.assertIsInstance(state, str)

        # Verify it can be decoded
        payload = verify_oauth_state(state)
        self.assertEqual(payload["type"], FLOW_TYPE_JIRA_ONBOARDING)
        self.assertEqual(payload["team_id"], team_id)
        self.assertIn("iat", payload)

    def test_create_jira_onboarding_state_without_team_id(self):
        """Test creating Jira onboarding state without team_id (allowed)."""
        # Jira onboarding has optional team_id
        state = create_oauth_state(FLOW_TYPE_JIRA_ONBOARDING)
    ...
    def test_create_jira_integration_state(self):
        """Test creating Jira integration state."""
        team_id = 789
        state = create_oauth_state(FLOW_TYPE_JIRA_INTEGRATION, team_id=team_id)

        self.assertIsInstance(state, str)

        # Verify it can be decoded
        payload = verify_oauth_state(state)
        self.assertEqual(payload["type"], FLOW_TYPE_JIRA_INTEGRATION)
        self.assertEqual(payload["team_id"], team_id)

    def test_create_jira_integration_state_requires_team_id(self):
        """Test that Jira integration flow requires team_id."""
        with self.assertRaises(ValueError) as ctx:
            create_oauth_state(FLOW_TYPE_JIRA_INTEGRATION)

        self.assertIn("team_id is required", str(ctx.exception))

    def test_verify_jira_integration_state_without_team_id(self):
        """Test that Jira integration state without team_id is rejected."""
        import base64
        import json
