...
class TestOnboardingSyncServiceDaysBack(TestCase):
    ...
    @patch("apps.integrations.services.onboarding_sync.asyncio.run")
    def test_sync_repository_accepts_days_back(self, mock_asyncio_run):
        ...
        mock_asyncio_run.return_value = {"prs_synced": 10}

        service = OnboardingSyncService(team=self.team, github_token="test-token")
        service.sync_repository(repo=self.repo, days_back=30)

        # Verify asyncio.run was called (async GraphQL sync was invoked)
        mock_asyncio_run.assert_called_once()

    @patch("apps.integrations.services.onboarding_sync.asyncio.run")
    def test_sync_repository_accepts_skip_recent(self, mock_asyncio_run):
