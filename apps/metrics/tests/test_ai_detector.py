...
class TestReplexicaPatterns(TestCase):
    """Tests for Replexica i18n AI tool detection patterns."""

    def test_replexica_ai_mention(self):
        """'Replexica AI' should detect as replexica."""
        text = "[**Replexica AI**](https://replexica.com) here with fresh translations!"
        result = detect_ai_in_text(text)
        self.assertTrue(result["is_ai_assisted"])
        self.assertIn("replexica", result["ai_tools"])

    def test_replexica_com_link(self):
        """'replexica.com' link should detect as replexica."""
        text = "Translations updated via replexica.com"
        result = detect_ai_in_text(text)
        self.assertTrue(result["is_ai_assisted"])
        self.assertIn("replexica", result["ai_tools"])

    def test_replexica_localization_engine(self):
        """'Replexica Localization Engine' should detect."""
        text = "Enhanced translations using Replexica Localization Engine"
        result = detect_ai_in_text(text)
        self.assertTrue(result["is_ai_assisted"])
        self.assertIn("replexica", result["ai_tools"])

    def test_at_replexica_mention(self):
        """'@replexica' username mention should detect."""
        text = "feat: update translations via @replexica"
        result = detect_ai_in_text(text)
        self.assertTrue(result["is_ai_assisted"])
        self.assertIn("replexica", result["ai_tools"])

    def test_lingodotdev_mention(self):
        """'@lingodotdev' (rebrand) should detect as replexica."""
        text = "feat: update translations via @lingodotdev"
        result = detect_ai_in_text(text)
        self.assertTrue(result["is_ai_assisted"])
        self.assertIn("replexica", result["ai_tools"])

    def test_replexica_bot_author(self):
        """replexica[bot] should be detected as AI author."""
        result = detect_ai_author("replexica[bot]")
        self.assertTrue(result["is_ai"])
        self.assertEqual(result["ai_type"], "replexica")

    def test_lingodotdev_bot_author(self):
        """lingodotdev[bot] should be detected as AI author (replexica rebrand)."""
        result = detect_ai_author("lingodotdev[bot]")
        self.assertTrue(result["is_ai"])
        self.assertEqual(result["ai_type"], "replexica")


class TestCodeRabbitAuthorPatterns(TestCase):
    """Tests for CodeRabbit-authored PR detection patterns."""

    def test_coderabbit_docstring_generation(self):
        """'Docstrings generation was requested' should detect CodeRabbit."""
        text = """
    ...
    def test_coderabbit_cannot_edit_note(self):
        """'CodeRabbit cannot perform edits' should detect CodeRabbit."""
        text = """
        <details>
        <summary>Note</summary><blockquote>
    ...
    def test_coderabbit_generated(self):
        """'generated by coderabbit' should detect CodeRabbit."""
        text = "This PR was generated by CodeRabbit"
        result = detect_ai_in_text(text)
        self.assertTrue(result["is_ai_assisted"])
        self.assertIn("coderabbit", result["ai_tools"])


class TestMintlifyAuthorPatterns(TestCase):
    """Tests for Mintlify docs AI tool detection patterns."""

    def test_mintlify_writer(self):
        """'Mintlify Writer' should detect as mintlify."""
        text = "Documentation generated by Mintlify Writer"
        result = detect_ai_in_text(text)
        self.assertTrue(result["is_ai_assisted"])
        self.assertIn("mintlify", result["ai_tools"])

    def test_mintlify_com_not_detected(self):
        """'mintlify.com' should NOT detect - too many false positives from docs infra."""
        # Many companies use Mintlify for docs, linking to it doesn't mean AI authored
        text = "Docs updated via mintlify.com automation"
        result = detect_ai_in_text(text)
