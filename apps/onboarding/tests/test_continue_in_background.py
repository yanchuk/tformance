...
NOTE: These tests require the integration_jira_enabled flag to be active.
When Jira is disabled, the button shows "Continue to Dashboard" instead.
See test_flag_skip.py for tests covering the flag-based navigation behavior.
"""
...
class TestContinueInBackgroundButton(TestCase):
    ...
    def setUp(self):
        ...
        # Ensure waffle flags exist (required for override_flag to work)
    ...
    def test_continue_button_links_to_connect_jira_in_visible_section(self):
        ...
        with override_flag("integration_jira_enabled", active=True):
            ...
            self.assertGreaterEqual(
                len(links),
                2,
                f"Expected at least 2 links to connect_jira, found {len(links)}",
            )
