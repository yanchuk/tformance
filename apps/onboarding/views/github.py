"""GitHub-related onboarding views.

Contains views for GitHub OAuth flow, organization selection, repository
selection, and sync progress tracking.
"""
...
# Import from parent package to enable test mocking at apps.onboarding.views.*
# Tests patch apps.onboarding.views.github_oauth and apps.onboarding.views.start_onboarding_pipeline
from apps.onboarding import views as _views
...
def github_connect(request):
    ...
    params = {
        ...
        "scope": _views.github_oauth.GITHUB_OAUTH_SCOPES,
        "state": state,
    }
    auth_url = f"{_views.github_oauth.GITHUB_OAUTH_AUTHORIZE_URL}?{urlencode(params)}"
...
def start_sync(request):
    ...
    # Idempotency check: don't start if pipeline is already running
    if team.pipeline_in_progress:
        return JsonResponse(
            {
