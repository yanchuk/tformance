...
class TeamMemberManagementViewTest(MessagesTestMixin, TestCase):
    ...
    def test_members_cant_change_own_role(self):
        for membership in [self.normal_membership, self.admin_membership]:
            ...
            if original_role == roles.ROLE_MEMBER:
                ...
                self.assertMessages(response, [Message(ERROR, "You aren't allowed to change your own role.")])
    ...
    def test_members_cant_remove_other_members(self):
        ...
        self.assertTrue(Membership.objects.filter(pk=self.normal_membership2.pk).exists())
